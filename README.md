...
Dla otrzymanej chmury punktów w przestrzeni fazowej konstruuję kompleksy symplicjalne, wykorzystując filtrację Vietorisa-Ripsa. Śledzimy ewolucję pierwszej liczby Bettiego ($\beta_1$), która w topologii zlicza jednowymiarowe "dziury". W kontekście biologicznym, taka stabilna, topologiczna dziura odpowiada prawidłowemu cyklowi leczenia pacjenta.<br>

Wysoka wartość persystencji dla $\beta_1$ daje nam kategoryczną odpowiedź, że pacjent poprawnie reaguje na terapię adaptacyjną, a atraktor posiada zamkniętą geometrię. Spadek tej wartości to obiektywny wskaźnik na przerwanie cyklu.<br>
...
Tnę szereg czasowy PSA na odcinki i za pomocą okna o określonej szerokości przechodzę przez sygnał, zapisując maksymalną żywotność (persystencję) jednowymiarowych cykli. Tworząc z tych wartości wykres w czasie, otrzymuję potwierdzenie na wystąpienie bifurkacji układu. Detektor topologiczny jednoznacznie wskazuje moment załamania się cyklu granicznego – punkt, w którym guz przestaje reagować na terapię i następuje ucieczka nowotworu. Eliminuje to potrzebę subiektywnej oceny wizualnej wykresów.<br>

## Wyniki Działania Algorytmu
Analiza została przeprowadzona na danych Pacjenta nr 7. Aby zachować pełną rzetelność i uniknąć sztucznego dopasowywania wyników, wszystkie wartości początkowe (ESS) oraz parametry modelu (współczynniki konkurencji, progi podawania leku) odpowiadają dokładnie oryginalnym danym klinicznym udostępnionym w publikacji Zhang et al. (2017).<br>

#### Rekonstrukcja Atraktora
W pierwszej kolejności wykorzystałam Twierdzenie Takensa do odtworzenia geometrii układu na podstawie samego poziomu PSA.<br>
![Phase Portrait | Persistence Diagram](Phase_Patient_7.png)

Lewy wykres (Phase Portrait): Przedstawia zrekonstruowaną przestrzeń fazową. Zielony punkt oznacza moment rozpoczęcia terapii. Widoczne fioletowe pętle to kolejne, prawidłowe cykle leczenia adaptacyjnego. Czerwony punkt na samym końcu trajektorii to moment ostatecznego załamania leczenia – trajektoria "wyłamuje się" z cyklu i dąży do nowego, niebezpiecznego punktu stałego.<br>
                                                                                                                                       Prawy wykres (Persistence Diagram): Potwierdza strukturę atraktora. Pomarańczowy punkt ($H_1$) położony wysoko nad przekątną to matematyczny indykator na istnienie silnego, stabilnego cyklu jedno-wymiarowego w danych.
#### Działanie Algorytmu "Sliding Window" w Czasie
Mając potwierdzoną obecność cykli, przepuściłam sygnał przez algorytm okna przesuwnego, aby zlokalizować dokładny moment mutacji i utraty skuteczności terapii.
![Topological Mutation Detector | PSA Tumor Dynamics](Detect_Patient_7.png)
Prawy wykres (PSA Tumor Dynamics): Z punktu widzenia klasycznej obserwacji medycznej, cykle pacjenta przez blisko 10 000 dni wydają się idealnie stabilne. Wykres PSA nie daje lekarzowi żadnych sygnałów ostrzegawczych o nadchodzącej ucieczce konkurencyjnej. Nagle guz wymyka się spod kontroli i stabilizuje się na poziomie 20 000, co oznacza całkowite przejęcie przestrzeni przez subpopulację lekooporną.<br>
Lewy wykres (Topological Mutation Detector): Algorytm bezbłędnie analizuje tę sytuację. Wykres jednoznacznie pokazuje wysoką, stabilną persystencję $H_1$, dopóki terapia działa. W okolicach 10 000. dnia następuje kategoryczny i gwałtowny spadek persystencji do zera. To w tym momencie detektor matematycznie pokazuje rozerwanie cyklu granicznego – układ uległ bifurkacji, a podawanie leku według dotychczasowego schematu straciło sens.
